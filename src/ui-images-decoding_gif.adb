...
package body UI.Images.Decoding_GIF is

   ----------
   -- Load --
   ----------

   subtype U8 is HAL.UInt8;
   subtype U16 is HAL.UInt16;
   ...
   procedure Load (image     : in Image_descriptor) is
      ...
      Cur_Pos      : Natural := Image.Bitmap'First;

      procedure Get_Byte(byte: out U8) is
      begin
         byte := Image.Bitmap (Cur_Pos);
         Cur_Pos := Cur_Pos + 1;
      end Get_Byte;
   ...
   begin -- Load
      ...
      Read_Intel(Descriptor.ImageLeft);
      Read_Intel(Descriptor.ImageTop);
      Read_Intel(Descriptor.ImageWidth);
      Read_Intel(Descriptor.ImageHeight);
      Get_Byte(Descriptor.Depth);
...
--        end if;
      pixel_mask:= U32(Image.Palette'Length - 1);
